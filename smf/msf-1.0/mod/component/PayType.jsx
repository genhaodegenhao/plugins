...
module.exports = React.createClass({
	...
	render: function () {
		...
		const dom_fft = <li className="pay-menu-item" onClick={() => {
		  			if(ftEnabled == 0) {
		  				return;
		  			}else {
		  				this.handleKuaiqianClick();
		  			}
		  		}}>
			  		<img className="pay-img" title="" src={ftEnabled == 1?__uri("../msf-1.0/res/images/fft-logo@2x.png?__inline"):__uri("../msf-1.0/res/images/fft-logo-hui@2x.png?__inline")}/>
		  			<div className="pay-menu-item-tips">
		  				<p className="pay-menu-item-name" style={ftEnabled == 0?disabledTextTop:null}>飞凡通支付</p>
		  				<p className="pay-menu-item-des-hot">{ ftEnabled == 1?(payTypeOpo.FFT?payTypeOpo.FFT.memo:''):''}</p>
		  				<p className="pay-menu-item-des-hot" style={disabledTextBottom}>{ftEnabled == 0?disabledTextInfoFT:''}</p>

		  			</div>
		  			<img className="pay-select" src={ftEnabled == 1?(this.state.isKuaiqian?select_active:select_disabled):__uri('../msf-1.0/res/images/Float_zhihui@2x.png?__inline')} />
		  		</li>;
