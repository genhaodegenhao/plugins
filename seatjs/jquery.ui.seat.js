...
;(function($){
		...
		for(var i=0;i<opts.data.length;i++){
			...
			for(var j=0;j<rd.length;j++){
				...
				if(rd[j].seat){
					...
					seat.attr("data",rd[j].seat.rowindex+"-"+rd[j].seat.colindex+"-"+rd[j].seat.rowname+"-"+rd[j].seat.colname);
	...
	function isActivitySeat(target,opts,s){
		...
		for(var i=0;i<opts.activitydata.length;i++){
			if(s.colname==opts.activitydata[i].orderseat.colname && s.rowname==opts.activitydata[i].orderseat.rowname){
				return true;
			}
		}
		return false;
	...
	function selectSeat(target,param){
		...
		if(status>0){
			if(status==1){
				...
				$.FloatingHint({"id":"FLOATINGHINT","holdTime":1000,"text":"亲最多选择"+opts.maxSelected+"个座位哦！"});
				//alert("亲最多选择6个座位哦");
			}
			return;
		}
